...
Int_t TKinFitter::fit() {
  ...
  do {
    ...
    if( TMath::IsNaN(currF) ) {
      std::cerr << "KinFitter - " << "The current value of F is NaN. Fit will be aborted.";
      _status = -10;
    }
    if( TMath::IsNaN(currS) ) {
      std::cerr << "KinFitter - " << "The current value of S is NaN. Fit will be aborted.";
      _status = -10;
    }
...
Bool_t TKinFitter::calcV() {
  ...
  try {
    ...
    std::cerr << "KinFitter - " << "Failed to invert covariance matrix V. Fit will be aborted.";
    _status = -10;
  }
...
Bool_t TKinFitter::calcVB() {
  ...
  try {
    ...
    std::cerr << "KinFitter - " << "Failed to invert matrix VB. Fit will be aborted.";
    _status = -10;
  }
...
Bool_t TKinFitter::calcVA() {
  ...
  try {
    ...
    std::cerr << "KinFitter - " << "Failed to invert matrix VA. Fit will be aborted.";
    _status = -10;
  }
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString& name) {
  ...
  for (Int_t iSheet = 0; iSheet < nCols; iSheet += colsPerSheet) {
    ...
    for (Int_t iCol = iSheet; iCol < nCols; iCol++) {
      if (iCol < iSheet+colsPerSheet) std::cout << std::setw(8) << iCol << "    |";}
    std::cout << "\n"
	 << topbar << " \n";
    for(Int_t iRow = 0; iRow < nRows; iRow++) {
      std::cout << std::setw(4) << iRow << " |";
      for (Int_t iCol = iSheet; iCol < nCols; iCol++) {
          if (iCol < iSheet+colsPerSheet ) std::cout << std::setw(12) << matrix(iRow, iCol) << " "; }
      std::cout << "\n";
    }
  }
